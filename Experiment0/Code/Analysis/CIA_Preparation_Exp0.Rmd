...
```{r load data Exp0, warning=FALSE, echo=FALSE}
# clear directory
rm(list=ls()) 
...
#### Experiment 0
#### PrepRT analysis
```{r Exp 0 prepRT analysis and plot, warning=FALSE, message=FALSE, echo=FALSE}
# remove outliers and calculate prepRT
prepRT0 = input_Exp0 %>%
  filter(prepRT > 100) %>%
  filter(prepRT < 20000) %>%
  filter(rt > 100) %>% 
...
prepRT0_GM = prepRT0 %>%
  summarise(
    prepRT = mean(prepRT))
kable(prepRT0_GM, caption = "PrepRT", digits = 2) 
...
kable(prepRT0_forpaper, caption = "PrepRT across all trials", digits = 2) 
``` 


\newpage 
#### Performance on image-alone trials
```{r Exp 0 ACC and exeRT image-alone trials, warning=FALSE, message=FALSE, echo=FALSE}
# ACC
...
\newpage 
#### Pair-split analysis
```{r Exp 0 pair-split analysis, warning=FALSE, message=FALSE, echo=FALSE}
...
### Plots for paper
```{r plots all, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r UPPSP, warning=FALSE, message=FALSE, echo=FALSE}
# check that each participant has 20 UPPSP responses
p_no_uppsp = table(UPPSP_Exp0$participant)
#p_no_uppsp

# calculate UPPSP score for each participant for each factor
# perseverance
UPPSP_Exp0_perseverance = UPPSP_Exp0 %>%
  filter(factor == "Perseverance")

UPPSP_Exp0_perseverance$resp = as.numeric(as.character(UPPSP_Exp0_perseverance$resp))

UPPSP_Exp0_perseverance_score = UPPSP_Exp0_perseverance %>%
  group_by(participant) %>%
  summarise(Perseverance_Score = mean(resp))


# premeditation
UPPSP_Exp0_premeditation = UPPSP_Exp0 %>%
  filter(factor == "Premeditation")

UPPSP_Exp0_premeditation$resp = as.numeric(as.character(UPPSP_Exp0_premeditation$resp))

UPPSP_Exp0_premeditation_score = UPPSP_Exp0_premeditation %>%
  group_by(participant) %>%
  summarise(Premeditation_Score = mean(resp))


# positive urgency
UPPSP_Exp0_posurgency = UPPSP_Exp0 %>%
  filter(factor == "Positive Urgency")

UPPSP_Exp0_posurgency$resp = as.numeric(as.character(UPPSP_Exp0_posurgency$resp))

UPPSP_Exp0_posurgency_score = UPPSP_Exp0_posurgency %>%
  group_by(participant) %>%
  summarise(PosUrgency_Score = mean(resp))


# negative urgency
UPPSP_Exp0_negurgency = UPPSP_Exp0 %>%
  filter(factor == "Negative Urgency")

UPPSP_Exp0_negurgency$resp = as.numeric(as.character(UPPSP_Exp0_negurgency$resp))

UPPSP_Exp0_negurgency_score = UPPSP_Exp0_negurgency %>%
  group_by(participant) %>%
  summarise(NegUrgency_Score = mean(resp))


# sensation seeking
UPPSP_Exp0_senseek = UPPSP_Exp0 %>%
  filter(factor == "Sensation Seeking")

UPPSP_Exp0_senseek$resp = as.numeric(as.character(UPPSP_Exp0_senseek$resp))

UPPSP_Exp0_senseek_score = UPPSP_Exp0_senseek %>%
  group_by(participant) %>%
  summarise(SenSeeking_Score = mean(resp))


# combine all UPPSP factors and prepRT
prepRT0_cor$Perseverance_Score = UPPSP_Exp0_perseverance_score$Perseverance_Score
prepRT0_cor$Premeditation_Score = UPPSP_Exp0_premeditation_score$Premeditation_Score
prepRT0_cor$PosUrgency_Score = UPPSP_Exp0_posurgency_score$PosUrgency_Score
prepRT0_cor$NegUrgency_Score = UPPSP_Exp0_negurgency_score$NegUrgency_Score
prepRT0_cor$SenSeeking_Score = UPPSP_Exp0_senseek_score$SenSeeking_Score


# save as csv
write.csv(prepRT0_cor, 'Exp0_prepRT_UPPSP', row.names = FALSE)
```